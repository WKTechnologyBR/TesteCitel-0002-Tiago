...
begin
  ...
  lQrPedido.SQL.Text := ' SELECT ped.id_pedido,                                                                                       ' +
                        ...
                        '      left join  cidade cid    on cid.id_cidade = cli.id_cidade                                                                             ' + lWhere;
...
begin
  ...
  try
    ...
    lQryPedido.ExecSQL(' INSERT INTO pedido (id_cliente, valor_total) VALUES (:id_cliente , :valor_total)', [pPedido.IdCliente, pPedido.ValorTotal]);
    pPedido.ID := FConexao.GetConexao.GetLastAutoGenValue('id_pedido');
    for lPedidoProduto in pPedido.ListaPedidoProduto do
    begin
      lQryPedido.ExecSQL(' INSERT INTO pedido_produto (id_pedido,id_produto,quantidade,valor_unidade) ' + ' VALUES (:id_pedido,:id_produto,:quantidade,:valor_unidade) ', [pPedido.ID, lPedidoProduto.IdProduto, lPedidoProduto.Quantidade, lPedidoProduto.ValorUnitario]);
    end;
...
begin
  ...
  try
    FConexao.GetConexao.StartTransaction;
    lQryPedido.ExecSQL('DELETE FROM pedido_produto WHERE id_pedido = :id_pedido', [pIdPedido]);
    lQryPedido.ExecSQL('DELETE FROM pedido WHERE id_pedido = :id_pedido', [pIdPedido]);
